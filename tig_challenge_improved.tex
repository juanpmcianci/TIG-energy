...
\subsection{PTDF Matrix Construction}

The Power Transfer Distribution Factor (PTDF) matrix encodes how nodal power injections distribute across transmission lines under the DC power flow approximation. Given a network with $n$ nodes and $L$ lines, the PTDF matrix $\Psi \in \mathbb{R}^{L \times n}$ is constructed as follows.

\begin{algorithm}[H]
\caption{PTDF Matrix Construction}
\begin{algorithmic}[1]
\State \textbf{Input:} Nodes $\mathcal{N} = \{1, \ldots, n\}$, lines $\mathcal{L}$ with endpoints $(i_\ell, j_\ell)$ and susceptances $b_\ell$, slack bus $s$
\State \textbf{Output:} PTDF matrix $\Psi \in \mathbb{R}^{L \times n}$
\State
\State \textbf{Step 1: Build bus susceptance matrix} $B \in \mathbb{R}^{n \times n}$
\For{each line $\ell = (i, j) \in \mathcal{L}$}
    \State $B_{ii} \gets B_{ii} + b_\ell$
    \State $B_{jj} \gets B_{jj} + b_\ell$
    \State $B_{ij} \gets B_{ij} - b_\ell$
    \State $B_{ji} \gets B_{ji} - b_\ell$
\EndFor
\State
\State \textbf{Step 2: Form reduced matrix by removing slack bus}
\State $\tilde{B} \gets B$ with row $s$ and column $s$ deleted \Comment{$(n-1) \times (n-1)$ matrix}
\State
\State \textbf{Step 3: Compute sensitivity matrix}
\State $\tilde{X} \gets \tilde{B}^{-1}$
\State Extend to full matrix: $X_{ik} = \tilde{X}_{ik}$ for $i,k \neq s$; $X_{sk} = X_{is} = 0$
\State
\State \textbf{Step 4: Compute PTDF entries}
\For{each line $\ell = (i, j) \in \mathcal{L}$}
    \For{each node $k \in \mathcal{N}$}
        \State $\Psi_{\ell k} \gets b_\ell \cdot (X_{ik} - X_{jk})$
    \EndFor
\EndFor
...
The resulting PTDF matrix satisfies $f_\ell = \sum_{k \in \mathcal{N}} \Psi_{\ell k} \cdot p_k$ for line flows $f_\ell$ given nodal injections $p_k$. The slack bus column is identically zero since injections at the slack do not induce flows (by convention, the slack absorbs any imbalance).

\subsection{Exogenous Nodal Injections}

The exogenous injection profile $p^{\text{exo}}_{i,t}$ represents background generation and load at each node, independent of battery operations. We generate these as a low-rank spatiotemporal process to capture realistic correlation structure.

\begin{definition}[Exogenous Injection Generation]
For each node $i \neq s$ and time step $t$:
\begin{equation}
    p^{\text{exo}}_{i,t} = \bar{p} \cdot \zeta_i + \gamma_p \sum_{k=1}^{2} w_{i,k} \cdot g_k(t) + \epsilon_{i,t},
\end{equation}
where:
\begin{itemize}
    \item $\bar{p}$ is the base load scale (MW), $\zeta_i \sim \mathcal{U}(-0.5, 0.5)$ is node-specific offset
    \item $w_{i,k} \sim \mathcal{U}(-0.5, 0.5)$ are random node loadings on temporal factors
    \item $g_1(t) = \sin(2\pi t / 96)$ and $g_2(t) = \sin(2\pi t / 48 + \pi/4)$ are diurnal patterns
    \item $\gamma_p$ scales the structured component
    \item $\epsilon_{i,t} \sim \mathcal{N}(0, \sigma^2_\epsilon)$ is idiosyncratic noise
\end{itemize}
The slack bus injection balances the system: $p^{\text{exo}}_{s,t} = -\sum_{i \neq s} p^{\text{exo}}_{i,t}$.
\end{definition}

After generation, injections are scaled to ensure baseline flows remain within a safety margin (typically 70\%) of line limits, leaving headroom for battery operations:
\begin{equation}
    p^{\text{exo}} \gets \min\left(1, \min_{\ell, t} \frac{0.7 \cdot \bar{F}_\ell \cdot \gamma_{\text{cong}}}{|f^{\text{exo}}_{\ell,t}|}\right) \cdot p^{\text{exo}}.
\end{equation}

\subsection{Battery Fleet Generation}

Each battery $b \in \mathcal{B}$ is placed uniformly at random on a non-slack node $\nu(b) \sim \mathcal{U}(\mathcal{N} \setminus \{s\})$. Battery parameters are scaled from nominal values using a heterogeneity mechanism controlled by parameter $h \in [0, 1]$.

\begin{definition}[Battery Heterogeneity]
For each battery $b$, draw $r_b \sim \mathcal{U}(0,1)$ and compute the scaling factor:
\begin{equation}
    M_b = 3^{h(2r_b - 1)}.
\end{equation}
The battery parameters are then:
\begin{align}
    \bar{E}_b &= \bar{E}^{\text{nom}} \cdot M_b, & \bar{P}^c_b = \bar{P}^d_b &= \bar{P}^{\text{nom}} \cdot M_b,
\end{align}
where $\bar{E}^{\text{nom}} = 100$ MWh and $\bar{P}^{\text{nom}} = 25$ MW are nominal values.
\end{definition}

When $h = 0$, all batteries are identical ($M_b = 1$). When $h = 1$, the scaling factor ranges over $[1/3, 3]$, producing up to a 9$\times$ spread in battery sizes across the fleet.

\subsection{Efficiency Parameters}

All batteries share common round-trip efficiency characteristics:
\begin{align}
    \eta^c &= 0.95, & \eta^d &= 0.95,
\end{align}
yielding a round-trip efficiency of $\eta = \eta^c \cdot \eta^d = 0.9025$. The state-of-charge dynamics for battery $b$ follow:
\begin{equation}
    E_{b,t+1} = E_{b,t} + \eta^c c_{b,t} \Delta t - \frac{d_{b,t} \Delta t}{\eta^d},
\end{equation}
with $\Delta t = 0.25$ hours (15-minute intervals) and SOC bounds at 10\% and 90\% of capacity.
...
For benchmarking solver performance, we provide a greedy heuristic baseline for Level 2. The algorithm operates myopically at each time step, making flow-aware dispatch decisions without lookahead optimization. The reference implementation is available in the repository at \texttt{tig-algorithms/src/energy\_arbitrage\_v2/level2\_greedy.rs}.

\subsection{Greedy Heuristic Overview}

The baseline uses threshold-based rules to determine charge/discharge actions:

\begin{enumerate}
    \item \textbf{Price Signal:} Compare the day-ahead price $\lambda^{\text{DA}}_{i,t}$ at each battery's node against configurable thresholds. If below the charge threshold (default: 95\% of mean), attempt to charge; if above the discharge threshold (default: 105\% of mean), attempt to discharge.

    \item \textbf{SOC Feasibility:} Compute maximum feasible charge/discharge given current SOC and battery limits.

    \item \textbf{Flow Adjustment:} After determining candidate actions for all batteries, check network flow feasibility. If any line constraint would be violated, iteratively reduce actions on batteries incident to congested lines until flows are within limits (with a safety margin, default: 85\% of limit).
\end{enumerate}

This greedy approach provides a tractable baseline that respects all constraints but does not exploit temporal arbitrage opportunities or coordinate across batteries optimally. Solvers are expected to significantly outperform this baseline by incorporating price forecasts, stochastic modeling, and inter-temporal coordination.
...
$\text{PTDF}$, $\Psi$ & Power transfer distribution factors \\
$f_\ell$ & Flow on line $\ell$ \\
$\overline{F}_\ell$ & Line flow limit \\
$\gamma_{\text{cong}}$ & Congestion scaling factor \\
$p^{\text{exo}}_{i,t}$ & Exogenous injection at node $i$, time $t$ \\
$M_b$ & Battery heterogeneity multiplier \\
$h$ & Fleet heterogeneity parameter $\in [0,1]$ \\
