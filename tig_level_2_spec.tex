...
\]
and $p_{i,t}$ for $i\ne s$ is as above. This formulation enforces the global balance constraint: at each moment, total power injected into the network must equal total power withdrawn. Rather than checking balance as a constraint, we \emph{define} the slack bus injection to absorb whatever imbalance exists among the other nodes, guaranteeing balance by construction. Given $p_{\cdot,t}$, line flows are computed using the PTDF mapping
...
\]
This construction ensures that $\mathrm{Corr}(\xi_{i,t},\xi_{j,t})=\rho_{\mathrm{sp}}$ for any pair of nodes $i\ne j$, while each $\xi_{i,t}$ remains a standard normal. The parameter $\rho_{\mathrm{sp}}$ thus directly controls the degree to which price shocks are system-wide versus node-specific.
...
This component models scarcity events---sudden, large price spikes that occur when supply is tight or unexpected outages occur. Independently for each node-time pair, with probability $\rho_{\mathrm{jump}}$ draw $Z\sim\mathrm{Pareto}(\alpha)$ (support $[1,\infty)$) and set $J_{i,t}:=\lambda^{\mathrm{DA}}_{i,t}\cdot Z$; otherwise set $J_{i,t}:=0$. The parameter $\rho_{\mathrm{jump}}$ controls the frequency of these events, while $\alpha>2$ controls tail heaviness (smaller $\alpha$ yields more extreme spikes).
...
\paragraph{Role of frictions.}
Without friction terms, the optimal strategy would exploit arbitrarily small price differences by charging and discharging constantly, cycling batteries at unrealistically high rates. The friction function $\phi_b(\cdot)$ penalizes such behavior, ensuring that only sufficiently large price spreads justify action and that battery usage remains physically plausible.
...
\end{equation}
The first term is a \emph{transaction cost} proportional to energy throughput, representing market fees, bid--ask spreads, or other per-MWh charges. The second term is a \emph{degradation cost} that penalizes deep cycling: the exponent $\beta_b>1$ makes the marginal cost of cycling increase with intensity, discouraging aggressive charge/discharge patterns that accelerate battery wear.
...
The challenge has two distinct sources of hardness: coupled feasibility under network constraints, and sequential decision-making under action-committed stochastic prices. Network coupling is primarily controlled by $(n,L)$ and the tightness scaling $\gamma_{\mathrm{cong}}$; portfolio complexity is controlled by $m$ and heterogeneity $h$ (dispersion of battery parameters); uncertainty is controlled by volatility $\sigma$ and jump tail parameters $(\rho_{\mathrm{jump}},\alpha)$. TIG runs five tracks, each fixing a parameter regime.

The five tracks need not form a strict ladder of computational difficulty; rather, they are designed to induce qualitatively different algorithmic regimes, encouraging solvers to develop flexible approaches that can adapt to varying conditions. Early tracks are correctness-oriented and only mildly congested, while later tracks feature frequent congestion, heavier tails, longer horizons, and materially heterogeneous fleets. This diversity tests whether algorithms can balance feasibility management, risk control, and spatiotemporal coordination across a range of scenarios.
...
Instances are generated from a single master seed and track parameters. This section provides the detailed algorithms used in the reference implementation.

\subsection{Network topology and PTDF matrix}
The generator first produces a connected network graph with $n$ nodes and $L$ lines. Connectivity is ensured by building a spanning tree (selecting $n-1$ edges to connect all nodes), then adding $L-(n-1)$ additional edges uniformly at random among non-existing pairs. Each line $\ell$ connecting nodes $i$ and $j$ is assigned a susceptance $b_\ell = b_{\mathrm{base}}\cdot(0.8 + 0.4\,r_\ell)$ where $r_\ell\sim\mathrm{Uniform}(0,1)$ and $b_{\mathrm{base}}=10$. Nominal flow limits are similarly perturbed: $\overline{F}^{\mathrm{nom}}_\ell = \overline{F}_{\mathrm{base}}\cdot(0.8+0.4\,r'_\ell)$ with $\overline{F}_{\mathrm{base}}=100$ MW.

The PTDF matrix is constructed via the standard DC power flow derivation:
\begin{enumerate}
\item \textbf{Build the bus susceptance matrix} $\mathbf{B}\in\mathbb{R}^{n\times n}$:
\[
B_{ii} = \sum_{\ell:\,i\in\ell} b_\ell, \qquad B_{ij} = -b_\ell \text{ if line } \ell \text{ connects } i \text{ and } j, \qquad B_{ij}=0 \text{ otherwise}.
\]
\item \textbf{Form the reduced matrix} $\mathbf{B}_{\mathrm{red}}\in\mathbb{R}^{(n-1)\times(n-1)}$ by deleting the row and column corresponding to the slack bus $s$.
\item \textbf{Invert} to obtain $\mathbf{X}_{\mathrm{red}}=\mathbf{B}_{\mathrm{red}}^{-1}$.
\item \textbf{Expand} back to $\mathbf{X}\in\mathbb{R}^{n\times n}$ by inserting zeros in the slack row and column.
\item \textbf{Compute PTDF entries}: for each line $\ell=(i,j)$ and injection node $k$,
\[
\mathrm{PTDF}_{\ell k} = b_\ell\,(X_{ik} - X_{jk}).
\]
\end{enumerate}

\subsection{Exogenous nodal injections $p^{\mathrm{exo}}_{i,t}$}
Exogenous injections are generated as a low-rank spatiotemporal process to produce realistic correlated load patterns:
\begin{enumerate}
\item \textbf{Define two temporal basis functions} (sinusoidal load curves):
\[
\varphi_1(t) = \sin\!\left(\frac{2\pi t}{96}\right), \qquad \varphi_2(t) = \sin\!\left(\frac{2\pi t}{48} + \frac{\pi}{4}\right).
\]
\item \textbf{Draw two spatial loading patterns}: for each node $i\ne s$, draw $\psi_1(i),\psi_2(i)\sim\mathrm{Uniform}(-0.5,0.5)$.
\item \textbf{Combine with noise}: for $i\ne s$,
\[
p^{\mathrm{exo}}_{i,t} = p_{\mathrm{base}}\,(r_{i,t}-0.5) + \gamma_{\mathrm{pat}}\bigl(\psi_1(i)\,\varphi_1(t)+\psi_2(i)\,\varphi_2(t)\bigr) + \gamma_{\mathrm{noise}}\,\varepsilon_{i,t},
\]
where $r_{i,t}\sim\mathrm{Uniform}(0,1)$, $\varepsilon_{i,t}\sim\mathcal{N}(0,1)$, and default scales are $p_{\mathrm{base}}=50$ MW, $\gamma_{\mathrm{pat}}=20$, $\gamma_{\mathrm{noise}}=2$.
\item \textbf{Balance at slack bus}:
\[
p^{\mathrm{exo}}_{s,t} = -\sum_{i\in\mathcal{N}\setminus\{s\}} p^{\mathrm{exo}}_{i,t}.
\]
\item \textbf{Rescale for feasibility}: compute flows under zero storage actions; if any $|f_{\ell,t}|>\gamma_{\mathrm{margin}}\,\overline{F}_\ell$ (with $\gamma_{\mathrm{margin}}=0.7$), uniformly scale all $p^{\mathrm{exo}}_{i,t}$ down and rebalance at slack. This ensures headroom for battery dispatch.
\end{enumerate}

\subsection{Day-ahead nodal prices $\lambda^{\mathrm{DA}}_{i,t}$}
Day-ahead prices combine a smooth system-wide baseline with node-specific offsets and temporal correlation:
\begin{enumerate}
\item \textbf{Generate a base price curve} $\bar{\lambda}_t$ using a Gaussian process with a periodic-plus-squared-exponential kernel:
\[
K(\tau) = \sigma_{\mathrm{per}}^2\exp\!\left(-\frac{2\sin^2(\pi|\tau|/T_{\mathrm{per}})}{\ell_{\mathrm{per}}^2}\right) + \sigma_{\mathrm{SE}}^2\exp\!\left(-\frac{\tau^2}{2\ell_{\mathrm{SE}}^2}\right),
\]
where $\tau=|t_1-t_2|\cdot\Delta t$ is the time difference in hours, $T_{\mathrm{per}}=24$ h, and default hyperparameters are $\sigma_{\mathrm{per}}=10$, $\ell_{\mathrm{per}}=2$, $\sigma_{\mathrm{SE}}=5$, $\ell_{\mathrm{SE}}=4$. The GP sample is obtained via Cholesky decomposition of the covariance matrix.
\item \textbf{Add a diurnal pattern}:
\[
\bar{\lambda}_t \leftarrow \bar{\lambda}_t + \lambda_0 + A\sin\!\left(\frac{2\pi\,(t\,\Delta t)}{24}-\frac{\pi}{2}\right),
\]
with $\lambda_0=50$ \$/MWh and $A=20$ \$/MWh.
\item \textbf{Draw node offsets} $\delta_i$: for each node $i$,
\[
\delta_i = 5\,(r_i - 0.5), \qquad r_i\sim\mathrm{Uniform}(0,1).
\]
\item \textbf{Generate AR(1) residuals} $\omega_{i,t}$: for each node $i$, initialize $\omega_{i,0}=0$ and evolve
\[
\omega_{i,t} = \rho_{\mathrm{AR}}\,\omega_{i,t-1} + \sqrt{1-\rho_{\mathrm{AR}}^2}\cdot 2\,z_{i,t}, \qquad z_{i,t}\sim\mathcal{N}(0,1),
\]
with $\rho_{\mathrm{AR}}=0.8$. (We use $\omega$ rather than $\eta$ to avoid confusion with the efficiency parameters $\eta^c_b,\eta^d_b$ in \eqref{eq:soc}.)
\item \textbf{Assemble nodal prices}:
\[
\lambda^{\mathrm{DA}}_{i,t} = \max\bigl\{\lambda_{\mathrm{DA,min}},\;\bar{\lambda}_t + \delta_i + \omega_{i,t}\bigr\}.
\]
\end{enumerate}

\subsection{Battery placement and parameters}
Batteries are placed uniformly at random across nodes (allowing multiple batteries per node). Nominal battery parameters $(\overline{E}^{\mathrm{nom}},\overline{P}^{\mathrm{nom}})$ are perturbed by the heterogeneity mechanism controlled by $h$ (Appendix~\ref{app:tracks}); friction parameters are set to track defaults unless explicitly varied.

\section{Baseline algorithm}
The reference implementation includes a \textbf{flow-aware greedy heuristic} as the baseline solver (see \texttt{level2\_greedy.rs} in the code repository). At each time step $t$, the algorithm:
\begin{enumerate}
\item Computes the average day-ahead price $\bar{\lambda}_t^{\mathrm{DA}}=\frac{1}{n}\sum_{i}\lambda^{\mathrm{DA}}_{i,t}$.
\item For each battery $b$ at node $\nu(b)$: if $\lambda^{\mathrm{DA}}_{\nu(b),t}<0.95\,\bar{\lambda}_t^{\mathrm{DA}}$ and SOC headroom permits, sets $u_{b,t}=-\overline{P}^c_b$ (full charge); if $\lambda^{\mathrm{DA}}_{\nu(b),t}>1.05\,\bar{\lambda}_t^{\mathrm{DA}}$ and SOC permits, sets $u_{b,t}=+\overline{P}^d_b$ (full discharge); otherwise idles.
\item Iteratively adjusts actions to satisfy network flow constraints: identifies the most violated line, scales down actions of batteries whose PTDF contributions worsen the violation, and repeats until feasible or a maximum iteration count is reached.
\item If violations persist, applies binary search over a global scaling factor $\alpha\in[0,1]$ to find the largest $\alpha$ such that $\alpha\cdot u_{\cdot,t}$ is flow-feasible.
\end{enumerate}
This greedy baseline is intentionally simple; it does not perform lookahead or stochastic optimization, making it a natural lower bound for more sophisticated solvers.
...
\paragraph{Rationale for the five tracks.}
The tracks are designed to induce qualitatively different algorithmic regimes rather than a strict ladder of computational difficulty; this encourages solvers to develop flexible, adaptive approaches.

\textbf{Track 1} is a correctness-and-baseline regime: the network is small, line limits are effectively nominal ($\gamma_{\mathrm{cong}}=1$), volatility is modest, and tail events are rare and thin-tailed, so many algorithm families can be competitive and feasibility is easy to maintain.

\textbf{Track 2} introduces meaningful congestion by tightening limits ($\gamma_{\mathrm{cong}}=0.8$) and increasing both stochasticity and fleet diversity, making spatial coordination materially valuable without turning the problem into a pure feasibility puzzle.

\textbf{Track 3} increases horizon and scale simultaneously, creating a regime in which solvers must manage multi-day energy carry, coordinate a larger portfolio, and remain robust to more frequent spikes; this tends to separate simple myopic heuristics from methods that internalize constraints and future opportunity cost.

\textbf{Track 4} increases network density and further tightens limits, increasing the frequency of binding line constraints and raising the importance of congestion-aware control; at the same time, heavier tails make risk management and degradation/friction tradeoffs central.

\textbf{Track 5} combines large network, large portfolio, tight limits, high volatility, and heavy tails, producing a regime where profitable operation requires consistently feasible coordination and disciplined response to rare but dominant scarcity events, while verification remains a straightforward replay.
