...
use tig_challenges::energy_arbitrage_v2::{
    constants, Challenge, Level2Challenge, PortfolioAction, SignedAction, Solution,
};
...
/// Solve Level 2 challenge using simplified Benders decomposition (entry point).
///
/// Pre-computes the full schedule via batch optimization, then replays it
/// through grid_optimize to apply the commitment chain.
pub fn solve_challenge(
    ...
    let precomputed = compute_schedule(challenge)?;

    let solution = challenge.grid_optimize(&|_challenge, state| {
        let t = state.time_step;
        if t < precomputed.len() {
            Ok(precomputed[t].clone())
        } else {
            Err(anyhow!("Schedule exhausted at step {}", t))
        }
    })?;
...
/// Pre-compute the schedule using batch optimization (Benders decomposition).
fn compute_schedule(challenge: &Level2Challenge) -> Result<Vec<PortfolioAction>> {
    compute_schedule_with_config(challenge, &DecompositionConfig::default())
}

/// Pre-compute with custom configuration.
fn compute_schedule_with_config(
    challenge: &Level2Challenge,
    config: &DecompositionConfig,
) -> Result<Vec<PortfolioAction>> {
...
pub fn help() {
    println!("Level 2 Benders Decomposition Solver for Energy Arbitrage v2");
    println!("Hyperparameters: none");
}
